...
function renderNews(items) {
    ...
    items.forEach(function (item) {
        html += "<tr style=\"cursor:pointer\" onclick=\"renderDetails(" + item.Id + ")\"><td>" + item.Title + "</td><td>" + item.Body + "</td><td>" + item.Created + "</td><td>" + item.Modified + "</td></tr>";
    });
    html += "</table>";
...
function renderDetails(id) {
    item = items.find(function (el) { return el.Id == id; });
    console.log("row was clicked: ", item);
    if (item != null) {
        var b = $("#id").val(item.Id);
...
//  });
//}
//# sourceMappingURL=NewsTable.aspx.js.map